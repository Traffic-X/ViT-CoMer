...
class InteractionBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.multiconv1 = Attention(dim, ca_num_heads=2)
        self.multiconv2 = Attention(dim, ca_num_heads=2)
        self.multiconv3 = Attention(dim, ca_num_heads=2)
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W):
        ...
        if self.use_injector:
            ...
            c_select1 = self.multiconv1(c_select1, H*2, W*2)
            c_select2 = self.multiconv2(c_select2, H, W)
            c_select3 = self.multiconv3(c_select3, H//2, W//2)
...
class SpatialPriorModule(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384):
        ...
        self.conv_4 = nn.Conv2d(embed_dim*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_8 = nn.Conv2d(embed_dim*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_16 = nn.Conv2d(embed_dim*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)

    def _spacial2channel(self, x):
        offsets = [[0, 0], [0, 1], [1, 0], [1, 1]]
        features = []
        for offset in offsets:
            a, b = offset
            features.append(x[:, :, a::2, b::2])
    ...
    def forward(self, x):
        ...
        c2_feat = c2 + self.conv_4(self._spacial2channel(c1))
        c3_feat = c3 + self.conv_8(self._spacial2channel(c2))
        c4_feat = c4 + self.conv_16(self._spacial2channel(c3))

        bs, dim, _, _ = c1.shape
        # c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s
        c2 = c2_feat.view(bs, dim, -1).transpose(1, 2)  # 8s
        c3 = c3_feat.view(bs, dim, -1).transpose(1, 2)  # 16s
        c4 = c4_feat.view(bs, dim, -1).transpose(1, 2)  # 32s
