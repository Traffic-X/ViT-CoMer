...
class SFP(BaseModule):
    ...
    def __init__(self,
        ...
        self.fc11 = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0, bias=False)
        self.fc22 = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0, bias=False)
        self.fc33 = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0, bias=False)
        self.fc44 = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0, bias=False)


    def forward(self, inputs):
        """Forward function."""
        (x, f1, f2, f3, f4) = inputs
        f11 = self.fc11(f1)
        f22 = self.fc22(f2)
        f33 = self.fc33(f3)
        f44 = self.fc44(f4)
        ...
        p4 = self.p4(x)  # p4
        p3 = self.p3(x)  # p3
        p5 = self.p5(x)  # p5->s/32
        p6 = self.p6(x)
        outs = [p3+f22, p4+f33, p5+f44, p6]
        if self.use_p2:
            outs = [self.p2(x) + f11] + outs
        return tuple(outs)